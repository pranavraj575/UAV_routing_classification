...
function single_vehicle_TSP_dwell_times(instance::AbstractInstance, 
    ...
    start_time = time()
    dwell_times=nesterov_gradient_descent(taui_arr,instance.alpha,1.)
    # dwell_times=gradient_descent(taui_arr, instance.alpha, beta1, beta2)
    end_time = time()

    if computation_time
        return tour, dwell_times, tour_cost, key_array, TSP_time, end_time - start_time
    end
...
function nesterov_gradient_descent(taui_arr,alpha,eta,tolerance=.0001)
